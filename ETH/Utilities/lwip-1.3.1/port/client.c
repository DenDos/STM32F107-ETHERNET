...
////////////////////////////	 
  ...
  data = *(((char *)p->payload)); */ 
   ...
   /* Connect the upcb  */
  udp_connect(upcb,&DestIPaddr,80);//UDP_SERVER_PORT IP_ADDR_ANY IP_ADDR_BROADCAST

   p = pbuf_alloc(PBUF_TRANSPORT, strlen(tdata), PBUF_POOL);
	 if (p!= NULL)
	 {
			/* copy data to pbuf */
			pbuf_take(p, tdata, strlen(tdata));
		/* send udp data */
			udp_send(upcb, p);
	 }
   ...
   udp_bind(upcb, IP_ADDR_ANY,UDP_CLIENT_PORT);//UDP_CLIENT_PORT
   	
	 err = udp_connect(upcb,&DestIPaddr,5555);
